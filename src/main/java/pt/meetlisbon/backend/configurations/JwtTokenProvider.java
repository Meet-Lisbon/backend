...
public class JwtTokenProvider {
    ...
    // todo change key location to env var
    @Value("${security.jwt.token.secret-key:secret-key}")
    private String secretKey;

    @Value("${security.jwt.token.expire-length:3600000}")
    private long validityInMilliseconds = 3600000; // 1h
    private final ServerUserDetails serverUserDetails;

    public JwtTokenProvider(ServerUserDetails serverUserDetails) {
        this.serverUserDetails = serverUserDetails;
    }
    ...
    public Authentication getAuthentication(String token) {
        UserDetails userDetails = serverUserDetails.loadUserByUsername(getUsername(token));
